...
import { CheckCircle, Clock, Lock, MessageSquare, XCircle } from "lucide-react";
...
export default function HRPortal() {
  ...
  useEffect(() => {
    ...
    const adminApprovedApplications = storedApplications.filter(
      (app: Application) => app.adminStatus === "approved"
    );
    setApplications(adminApprovedApplications);
    setFilteredApplications(
      adminApprovedApplications.filter(
        (app: Application) => app.hrStatus === "pending"
      )
    );
  }, []);
  ...
  const updateApplicationStatus = (
    id: string,
    hrStatus: "approved" | "rejected" | "contacted",
    notes = ""
  ) => {
    ...
    // Update local state
    const adminApprovedApplications = updatedApplications.filter(
      (app: Application) => app.adminStatus === "approved"
    );
    setApplications(adminApprovedApplications);

    if (selectedApplication?.id === id) {
      const updatedApp = adminApprovedApplications.find(
  ...
  const stats = {
    ...
    contacted: applications.filter((app) => app.hrStatus === "contacted")
      .length,
            ...
            <div className="flex items-center gap-4">
              <div>
                ...
                <p className="text-slate-600 mt-1">
                  Review approved applications and contact candidates
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          >
                            ...
                            {application.hrStatus === "pending"
                              ? "Pending Review"
                              : application.hrStatus.charAt(0).toUpperCase() +
                                application.hrStatus.slice(1)}
                          </Badge>
                        </div>
            ...
            {actionType === "contact" ? (
              <div className="space-y-4">
                <p className="text-slate-600">
                  Send an email to {selectedApplication.fullName} at{" "}
                  {selectedApplication.email}:
                </p>
                <div>
